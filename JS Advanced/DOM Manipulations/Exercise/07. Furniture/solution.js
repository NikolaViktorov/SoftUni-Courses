function solve() {
   const tbody = document.querySelector('tbody');
    ...
    generate.addEventListener('click', onGnerate);
    buy.addEventListener('click', onBuy);


    function onGnerate(e) {
      const data = JSON.parse(input.value);

      for(let item of data) {
        const row = document.createElement('tr');
        const html = `<tr><td><img src="${item.img}"></td><td><p>${item.name}</p><td><p>${item.price}</p></td><td><p>${item.decFactor}</p></td><td><input type="checkbox"/></td></tr>`;
    row.innerHTML = html;
    ...
    function onBuy(e) {
      ...
      const result = [`Bought furniture: ${boughtItems.map(i => i.name).join(', ')}`,
